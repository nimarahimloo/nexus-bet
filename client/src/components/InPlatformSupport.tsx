...
export function InPlatformSupport({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const chatMutation = trpc.support.chat.useMutation({
    ...
    onError: () => setMessages((current) => [...current, { role: "assistant", content: "پاسخ پشتیبانی موقتاً دریافت نشد. لطفاً دوباره تلاش کن." }]),
  });
  if (!open) return null;
  const send = (content: string) => {
    const next = [...messages, { role: "user" as const, content }];
    setMessages(next);
    chatMutation.mutate({ messages: next.filter((message) => message.role !== "system").map((message) => ({ role: message.role as "user" | "assistant", content: message.content })) });
  };
