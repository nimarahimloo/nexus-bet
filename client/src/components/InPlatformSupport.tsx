...
export function InPlatformSupport({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return <div className="support-layer" role="presentation" onMouseDown={onClose}><section id="nexus-support-dialog" className="support-panel glass-panel" role="dialog" aria-modal="true" aria-label="پشتیبانی هوشمند Nexus" onMouseDown={(event) => event.stopPropagation()}><header className="support-head"><span className="support-avatar"><Bot size={19} /></span><div><span className="support-kicker"><i aria-hidden="true" /> NEXUS ASSIST</span><b>پشتیبانی هوشمند</b><small>{isAuthenticated ? "اتصال خواندنی به اطلاعات حساب فعال است" : "برای اطلاعات حساب، ابتدا وارد شوید"}</small></div><button onClick={onClose} aria-label="بستن پشتیبانی"><X size={19} /></button></header><AIChatBox className="nexus-support-chat" height="min(62dvh, 560px)" messages={messages} onSendMessage={send} isLoading={chatMutation.isPending} placeholder="سؤال خودت را بنویس…" emptyStateMessage="در مورد مسابقات، کیف پول، بلیت یا حساب بپرس." suggestedPrompts={[...(contextPrompt ? [contextPrompt] : []), "چطور بلیت ثبت کنم؟", "وضعیت کیف پول را کجا ببینم؟", "گردونه شانس چطور کار می‌کند؟"]} /><footer className="support-foot"><Headphones size={14} /> پاسخ‌های Nexus AI راهنمای عمومی پلتفرم هستند؛ عملیات مالی از داخل چت انجام نمی‌شود.</footer></section></div>;
}
