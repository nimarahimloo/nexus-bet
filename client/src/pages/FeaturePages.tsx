...
export function RewardsPage() {
  ...
  return <PageShell eyebrow="پاداش‌ها" title="هر روز، یک شانس واقعی" description="یک spin روزانه؛ نتیجه در backend و پاداش در ledger." heroImage="/manus-storage/nexus-bet-rewards-hero-v3_92731f27.png">
    <div className="data-state preview-notice"><ShieldCheck size={15} /><span>نتیجه و سقف روزانه از backend.</span></div>
    ...
    <div className="activity-reward-card glass-panel"><div className="section-heading"><div><span className="section-kicker">پاداش فعالیت</span><h2>امتیاز روزانه</h2></div><span className="sample-chip">سقف روزانه</span></div><p className="data-state">پاداش محدود برای فعالیت مسئولانه؛ مستقل از مبلغ شرط.</p>{!isAuthenticated ? <p className="data-state">برای دریافت پاداش، وارد حساب شو.</p> : activityQuery.isLoading ? <p className="data-state">در حال دریافت فعالیت‌های امروز…</p> : <div className="activity-task-list">{(activityQuery.data?.tasks ?? []).map((task) => <div className="activity-task-row" key={task.code}><div><b>{task.label}</b><span>{task.description}</span></div><strong>{formatFaDecimal(task.amount)} USDT</strong><button className="outline-cta" disabled={task.claimed || activityMutation.isPending} onClick={() => activityMutation.mutate({ activityCode: task.code })}>{task.claimed ? "دریافت شد" : activityMutation.isPending ? "در حال ثبت…" : "دریافت پاداش"}</button></div>)}</div>}{activityMutation.error && <p className="inline-alert">{activityMutation.error.message}</p>}</div>
    <div className="reward-odds glass-panel"><div className="section-heading"><div><span className="section-kicker">شفاف و قابل‌بررسی</span><h2>احتمال هر جایزه</h2></div><span className="sample-chip">۱۰۰٪ مجموع</span></div><p className="data-state">احتمال‌ها پیش از spin نمایش داده می‌شوند.</p><div className="reward-odds-list">{segments.map((segment) => <div className="reward-odds-row" key={segment.code}><span>{segment.label}</span><strong>{formatFaDecimal(segment.chancePercent, segment.chancePercent % 1 ? 1 : 0)}٪</strong><i><b style={{ width: `${segment.chancePercent}%` }} /></i></div>)}</div></div>
    ...
    <div className="reward-rules glass-panel"><CircleHelp size={19} /><div><b>منبع حقیقت پاداش</b><p>نتیجه و تغییر موجودی به ledger متصل است.</p></div><ShieldCheck size={18} /></div>
  </PageShell>;
}
