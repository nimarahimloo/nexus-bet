...
const featureCards = [
  { title: "بونوس‌های فعال", detail: "پیشنهادها را فقط با شرایط معتبر و قابل‌پیگیری ببین.", icon: BadgePercent, href: "/promotions", image: "/manus-storage/nexus-bet-promotions-hero-v3_65559fc7.png" },
  { title: "تورنمنت‌ها", detail: "رقابت‌ها پس از اتصال جدول امتیاز و قوانین رسمی نمایش داده می‌شوند.", icon: Trophy, href: "/tournaments", image: "/manus-storage/nexus-bet-tournaments-hero-v3_d13dd980.png" },
  { title: "گردونهٔ روزانه", detail: "یک spin واقعی در روز؛ نتیجه و پاداش در ledger ثبت می‌شود.", icon: Gift, href: "/rewards", image: "/manus-storage/nexus-bet-rewards-hero-v3_92731f27.png" },
  { title: "کازینو و بازی‌ها", detail: "بازی‌های قابل‌ورود باید از catalog واقعی سرویس خوانده شوند.", icon: Layers3, href: "/casino", image: "/manus-storage/nexus-bet-casino-hero-v3_e26e607d.png" },
];
...
export function RewardsPage() {
  ...
  return <PageShell eyebrow="پاداش‌ها" title="هر روز، یک شانس واقعی" description="نتیجه در backend تعیین می‌شود، فقط یک spin در روز مجاز است و هر پاداش USDT همراه با رکورد ledger به کیف پول اضافه می‌شود." heroImage="/manus-storage/nexus-bet-rewards-hero-v3_92731f27.png">
    ...
    <div className="reward-wheel-card glass-panel">
      ...
      <div className="wheel-copy"><span className="sample-chip">{isAuthenticated ? (statusQuery.data?.canSpin ? "امروز آماده‌ای" : "امروز استفاده شد") : "نیازمند ورود"}</span><h2>{result ? result.type === "usdt" ? `${formatFaDecimal(result.amount)} USDT به کیف پولت اضافه شد` : result.label : "شانس را به موجودی واقعی وصل کن"}</h2><p>{result ? "نتیجه از backend برگشته و در تاریخچه ثبت شده است." : "نتیجه در سرور انتخاب می‌شود و پاداش نقدی، در صورت برد، اتمیک به موجودی USDT اضافه خواهد شد."}</p><div className="wheel-actions"><button className="primary-cta" disabled={!canSpin} onClick={() => isAuthenticated ? spinMutation.mutate() : openAuthModal()}>{!isAuthenticated ? "ورود برای چرخاندن" : spinMutation.isPending ? "در حال ثبت نتیجه…" : statusQuery.data?.canSpin ? "چرخاندن گردونه" : "فردا دوباره امتحان کن"} {!isAuthenticated ? <LockKeyhole size={15} /> : <Gift size={15} />}</button>{!isAuthenticated && <button className="text-link" onClick={() => openAuthModal()}>ورود به Nexus Bet <ArrowLeft size={15} /></button>}</div>{spinMutation.error && <p className="inline-alert">{spinMutation.error.message}</p>}</div>
    </div>
    <div className="activity-reward-card glass-panel"><div className="section-heading"><div><span className="section-kicker">درآمد محدود از فعالیت</span><h2>هر روز بدون شرط هم امتیاز بگیر</h2></div><span className="sample-chip">سقف روزانه</span></div><p className="data-state">این پاداش درآمد تضمینی نیست؛ فقط برای فعالیت‌های مشخص و مسئولانه در backend ثبت می‌شود و به‌دنبال‌کردن باخت یا افزایش مبلغ شرط وابسته نیست.</p>{!isAuthenticated ? <p className="data-state">برای مشاهده و دریافت پاداش فعالیت وارد حساب شوید.</p> : activityQuery.isLoading ? <p className="data-state">در حال دریافت فعالیت‌های امروز…</p> : <div className="activity-task-list">{(activityQuery.data?.tasks ?? []).map((task) => <div className="activity-task-row" key={task.code}><div><b>{task.label}</b><span>{task.description}</span></div><strong>{formatFaDecimal(task.amount)} USDT</strong><button className="outline-cta" disabled={task.claimed || activityMutation.isPending} onClick={() => activityMutation.mutate({ activityCode: task.code })}>{task.claimed ? "دریافت شد" : activityMutation.isPending ? "در حال ثبت…" : "دریافت پاداش"}</button></div>)}</div>}{activityMutation.error && <p className="inline-alert">{activityMutation.error.message}</p>}</div>
    <div className="reward-odds glass-panel"><div className="section-heading"><div><span className="section-kicker">شفاف و قابل‌بررسی</span><h2>احتمال هر جایزه</h2></div><span className="sample-chip">۱۰۰٪ مجموع</span></div><p className="data-state">انتخاب نهایی تصادفی و فقط در backend انجام می‌شود؛ احتمال‌ها قبل از چرخاندن نمایش داده می‌شوند.</p><div className="reward-odds-list">{segments.map((segment) => <div className="reward-odds-row" key={segment.code}><span>{segment.label}</span><strong>{formatFaDecimal(segment.chancePercent, segment.chancePercent % 1 ? 1 : 0)}٪</strong><i><b style={{ width: `${segment.chancePercent}%` }} /></i></div>)}</div></div>
...
export function CasinoPage() {
  ...
  return <PageShell eyebrow="مرکز بازی‌ها" title="کتابخانهٔ بازی از catalog واقعی" description="فقط بازی‌هایی نمایش داده می‌شوند که catalog backend آن‌ها را فعال و قابل‌ورود اعلام کرده باشد." heroImage="/manus-storage/nexus-bet-casino-hero-v3_e26e607d.png">
