    ...
    chat: publicProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        try {
          ...
          const EMPTY_LLM_CONTENT = "پاسخ هوشمند در این لحظه از سرویس AI دریافت نشد";
          return { reply: typeof content === "string" && content.trim() ? content : EMPTY_LLM_CONTENT };
        } catch (error) {
    ...
    smartPicks: publicProcedure
      .input(z.object({
        candidates: z.array(z.object({
          ...
          status: z.string(),
          popularity: z.number().finite().min(0).max(100),
        })).max(40),
      }))
      .query(({ input }) => rankSmartPicks(input.candidates)),
  }),
});
