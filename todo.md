...
- [ ] تبدیل ساختار تک‌صفحه‌ای به مسیرهای جدا برای خانه، مسابقات، کیف پول، Nexus AI، VIP و حساب کاربری.
- [x] حذف modal تأیید سن از بارگذاری اولیه و انتقال آن به مسیر مسئولانه/شرایط استفاده.
- [ ] تولید و استفاده از hero مستقل برای هر صفحه با تصاویر مختص همان بخش.
- [ ] یکپارچه‌سازی کامل اعداد فارسی و وزن‌های فونت ارسالی در همهٔ صفحات و کارت‌ها.
- [ ] ساخت صفحهٔ مجزای انفجار با ضریب زنده، وضعیت بازی، تاریخچهٔ دورها و bet slip مستقل.
- [ ] اتصال مسابقات و وضعیت‌های live به API ورزشی واقعی با مدیریت secret، loading و خطا.
- [ ] بازبینی موبایل‌فرست همهٔ مسیرها و تست مسیرهای اصلی پیش از checkpoint جدید.
